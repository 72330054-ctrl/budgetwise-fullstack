...
function Transaction() {
    ...
    useEffect(() => {
        ...
        const fetchAllAcounts = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/transactions/${user.id}`);
                setTransactions(res.data);
                setFilteredTransactions(res.data); // initial display
            } catch (err) {
                ...
                {filteredTransactions.map((t) => (
                    <div className="col-md-4" key={t.id}>
                        <div className="transaction-card p-3">
                            ...
                            <p><strong>Category:</strong> {t.c_name}</p>
                            <p><strong>Acount:</strong> {t.acount_name}</p>
