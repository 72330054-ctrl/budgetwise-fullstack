...
function Notifications() {
    ...
    useEffect(() => {
        ...
        const fetchNotifications = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/notification/${user.id}`);
    ...
    const handlePay = async (billId) => {
        try {
            await axios.post(`http://localhost:5000/bill/pay/${billId}`);
    ...
    const handleDelete = async (billId) => {
        try {
            await axios.delete(`http://localhost:5000/bill/delete/${billId}`);
