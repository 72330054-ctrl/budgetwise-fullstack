...
function Budget() {
    ...
    useEffect(() => {
        if (!user?.id) return;
        axios
            .get(`http://localhost:5000/budget/${user.id}`)
    ...
    const handleUpdate = async (budget) => {
        ...
        try {
            await axios.post(`http://localhost:5000/budget/update/${budget.id}`, formValues);
            setBudgets(prev =>
                prev.map(b =>
                    b.id === budget.id
                        ? { ...b, ...formValues }
    ...
    const handleDetails = async (budget) => {
        try {
            const res = await axios.get(`http://localhost:5000/usedbudget/${budget.id}`);
            ...
            Swal.fire({
                ...
                didOpen: () => {
                    window.deleteTransaction = async (id) => {
                        try {
                            await axios.delete(`http://localhost:5000/usedbudget/delete/${id}`);
                            Swal.fire("Deleted!", "Transaction removed successfully.", "success");
                            const updatedRes = await axios.get(`http://localhost:5000/usedbudget/${budget.id}`);
